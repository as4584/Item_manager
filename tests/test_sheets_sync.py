...
def test_sync_sheets_full_demo_mode(client, monkeypatch):
    ...
    with patch('src.infra.sheets_repo.SheetsRepository.get_config', return_value={'LowStockThreshold': 5}) as _cfg:
        with patch('src.infra.sheets_repo.SheetsRepository.update_inventory') as upd:
            with patch('src.infra.sheets_repo.SheetsRepository.update_restock_list') as upd_restock:
